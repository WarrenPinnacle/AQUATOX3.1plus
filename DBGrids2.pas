...
{MODIFIED BY JSC TO ALLOW FOR FIXED COLUMN}
...
type
  ...
  protected
    ...
    function  GetGrid: TCustomDBGrid2;
  ...
  public
    ...
    procedure RestoreDefaults; virtual;
    property  Grid: TCustomDBGrid2 read GetGrid;
  ...
  public
    constructor Create(Grid: TCustomDBGrid2; ColumnClass: TColumnClass);
    ...
    property Grid: TCustomDBGrid2 read FGrid;
  ...
  public
    constructor Create(AGrid: TCustomDBGrid2);
    destructor Destroy; override;
  ...
  TCustomDBGrid2 = class(TCustomGrid)
  private
...
const
  ...
  bmEdit2 = 'DBEDIT2';
  bmInsert2 = 'DBINSERT2';
  bmMultiDot2 = 'DBMULTIDOT2';
  bmMultiArrow2 = 'DBMULTIARROW2';
...
begin
  ...
  begin
    ...
    if Accept then
      if ActiveList = DataList then
        with TCustomDBGrid2(Grid), Columns[SelectedIndex].Field do
        begin
...
begin
  TCustomDBGrid2(Grid).EditButtonClick;
end;
...
constructor TGridDataLink.Create(AGrid: TCustomDBGrid2);
begin
  inherited Create;
  FGrid := AGrid;
  VisualControl := True;
...
begin
  if FModified and Assigned(Field) and (Field.FieldKind = fkData) and
     (FGrid.SelectedField <> Field) then
    FGrid.UpdateData;
...
function TColumn.GetGrid: TCustomDBGrid2;
begin
  if Assigned(Collection) and (Collection is TDBGridColumns) then
...
constructor TDBGridColumns.Create(Grid: TCustomDBGrid2; ColumnClass: TColumnClass);
begin
...
constructor TBookmarkList.Create(AGrid: TCustomDBGrid2);
begin
  inherited Create;
...
constructor TCustomDBGrid2.Create(AOwner: TComponent);
var
  Bmp: TBitmap;
begin
  ...
  Bmp := TBitmap.Create;
  try
    Bmp.LoadFromResourceName(HInstance, bmArrow2);
    FIndicators := TImageList.CreateSize(Bmp.Width, Bmp.Height);
    FIndicators.AddMasked(Bmp, clWhite);
    Bmp.LoadFromResourceName(HInstance, bmEdit2);
    FIndicators.AddMasked(Bmp, clWhite);
    Bmp.LoadFromResourceName(HInstance, bmInsert2);
    FIndicators.AddMasked(Bmp, clWhite);
    Bmp.LoadFromResourceName(HInstance, bmMultiDot2);
    FIndicators.AddMasked(Bmp, clWhite);
    Bmp.LoadFromResourceName(HInstance, bmMultiArrow2);
    FIndicators.AddMasked(Bmp, clWhite);
  finally
    Bmp.Free;
  end;
...
destructor TCustomDBGrid2.Destroy;
begin
  FColumns.Free;
  FColumns := nil;
...
function TCustomDBGrid2.AcquireFocus: Boolean;
begin
  Result := True;
...
function TCustomDBGrid2.RawToDataColumn(ACol: Integer): Integer;
begin
  Result := ACol - FIndicatorOffset;
end;

function TCustomDBGrid2.DataToRawColumn(ACol: Integer): Integer;
begin
  Result := ACol + FIndicatorOffset;
end;

function TCustomDBGrid2.AcquireLayoutLock: Boolean;
begin
  Result := (FUpdateLock = 0) and (FLayoutLock = 0);
  if Result then BeginLayout;
end;

procedure TCustomDBGrid2.BeginLayout;
begin
  BeginUpdate;
...
procedure TCustomDBGrid2.BeginUpdate;
begin
  Inc(FUpdateLock);
end;

procedure TCustomDBGrid2.CancelLayout;
begin
...
function TCustomDBGrid2.CanEditAcceptKey(Key: Char): Boolean;
begin
  with Columns[SelectedIndex] do
    Result := FDatalink.Active and Assigned(Field) and Field.IsValidChar(Key);
end;

function TCustomDBGrid2.CanEditModify: Boolean;
begin
  Result := False;
...
function TCustomDBGrid2.CanEditShow: Boolean;
begin
  Result := (LayoutLock = 0) and inherited CanEditShow;
end;

procedure TCustomDBGrid2.CellClick(Column: TColumn);
begin
  if Assigned(FOnCellClick) then FOnCellClick(Column);
end;

procedure TCustomDBGrid2.ColEnter;
begin
  UpdateIme;
  if Assigned(FOnColEnter) then FOnColEnter(Self);
end;

procedure TCustomDBGrid2.ColExit;
begin
  if Assigned(FOnColExit) then FOnColExit(Self);
end;

procedure TCustomDBGrid2.ColumnMoved(FromIndex, ToIndex: Longint);
begin
...
procedure TCustomDBGrid2.ColWidthsChanged;
var
  I: Integer;
begin
...
function TCustomDBGrid2.CreateColumns: TDBGridColumns;
begin
  Result := TDBGridColumns.Create(Self, TColumn);
end;

function TCustomDBGrid2.CreateDataLink: TGridDataLink;
begin
  Result := TGridDataLink.Create(Self);
end;

function TCustomDBGrid2.CreateEditor: TInplaceEdit;
begin
  Result := TDBGridInplaceEdit.Create(Self);
end;

procedure TCustomDBGrid2.CreateWnd;
begin
...
procedure TCustomDBGrid2.DataChanged;
begin
...
procedure TCustomDBGrid2.DefaultHandler(var Msg);
var
  P: TPopupMenu;
  Cell: TGridCoord;
begin
...
procedure TCustomDBGrid2.DeferLayout;
var
  M: TMsg;
begin
...
procedure TCustomDBGrid2.DefineFieldMap;
var
  I: Integer;
begin
...
function TCustomDBGrid2.UseRightToLeftAlignmentForField(const AField: TField;
  Alignment: TAlignment): Boolean;
begin
  Result := False;
...
procedure TCustomDBGrid2.DefaultDrawDataCell(const Rect: TRect; Field: TField;
...
procedure TCustomDBGrid2.DefaultDrawColumnCell(const Rect: TRect;
...
procedure TCustomDBGrid2.ReadColumns(Reader: TReader);
begin
  Columns.Clear;
  Reader.ReadValue;
...
procedure TCustomDBGrid2.WriteColumns(Writer: TWriter);
begin
...
procedure TCustomDBGrid2.DefineProperties(Filer: TFiler);
var
  StoreIt: Boolean;
...
begin
  ...
  if Filer.Ancestor = nil then
    ...
    if vState <> TCustomDBGrid2(Filer.Ancestor).Columns.State then
      StoreIt := True
    else
      StoreIt := (vState = csCustomized) and
        (not CollectionsEqual(Columns, TCustomDBGrid2(Filer.Ancestor).Columns, Self, TCustomDBGrid2(Filer.Ancestor)));

  Filer.DefineProperty('Columns', ReadColumns, WriteColumns, StoreIt);
end;

function TCustomDBGrid2.ColumnAtDepth(Col: TColumn; ADepth: Integer): TColumn;
begin
  Result := Col;
...
function TCustomDBGrid2.CalcTitleRect(Col: TColumn; ARow: Integer;
  var MasterCol: TColumn): TRect;
var
  I,J: Integer;
  InBiDiMode: Boolean;
...
procedure TCustomDBGrid2.DrawCell(ACol, ARow: Longint; ARect: TRect; AState: TGridDrawState);
var
  FrameOffs: Byte;
  MasterCol: TColumn;
  TitleRect: TRect;
      ...
      try
        ...
        If ACol<1 then {jsc}
          Begin
            ...
            Brush.Color := MasterCol.Title.Color; {JSC}
            Font := MasterCol.Title.Font; {JSC}
          End;
        if Highlight then
        begin
...
procedure TCustomDBGrid2.DrawDataCell(const Rect: TRect; Field: TField;
  State: TGridDrawState);
begin
  if Assigned(FOnDrawDataCell) then FOnDrawDataCell(Self, Rect, Field, State);
end;

procedure TCustomDBGrid2.DrawColumnCell(const Rect: TRect; DataCol: Integer;
  Column: TColumn; State: TGridDrawState);
begin
...
procedure TCustomDBGrid2.EditButtonClick;
begin
...
procedure TCustomDBGrid2.EditingChanged;
begin
  if dgIndicator in Options then InvalidateCell(0, FSelRow);
end;

procedure TCustomDBGrid2.EndLayout;
begin
  if FLayoutLock > 0 then
  begin
    try
...
procedure TCustomDBGrid2.EndUpdate;
begin
  if FUpdateLock > 0 then
    Dec(FUpdateLock);
end;

function TCustomDBGrid2.GetColField(DataCol: Integer): TField;
begin
  Result := nil;
...
function TCustomDBGrid2.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

function TCustomDBGrid2.GetEditLimit: Integer;
begin
  Result := 0;
...
function TCustomDBGrid2.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
...
function TCustomDBGrid2.GetEditStyle(ACol, ARow: Integer): TEditStyle;
var
  Column: TColumn;
  MasterField: TField;
begin
...
function TCustomDBGrid2.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
  if FDatalink.Active then
    with Columns[RawToDataColumn(ACol)] do
...
function TCustomDBGrid2.GetFieldCount: Integer;
begin
  Result := FDatalink.FieldCount;
end;

function TCustomDBGrid2.GetFields(FieldIndex: Integer): TField;
begin
  Result := FDatalink.Fields[FieldIndex];
end;

function TCustomDBGrid2.GetFieldValue(ACol: Integer): string;
var
  Field: TField;
begin
  Result := '';
...
function TCustomDBGrid2.GetSelectedField: TField;
var
  Index: Integer;
begin
  Index := SelectedIndex;
...
function TCustomDBGrid2.GetSelectedIndex: Integer;
begin
  Result := RawToDataColumn(Col);
end;

function TCustomDBGrid2.HighlightCell(DataCol, DataRow: Integer;
...
procedure TCustomDBGrid2.KeyDown(var Key: Word; Shift: TShiftState);
var
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          NextRow(False);
          ACol := FIndicatorOffset + 1; {jsc}
        end
        else if ACol < FIndicatorOffset + 1 then {jsc}
        begin
          PriorRow(False);
          ACol := ColCount - FIndicatorOffset + 1;    {jsc}
        end;
...
procedure TCustomDBGrid2.KeyPress(var Key: Char);
begin
  FIsESCKey := False;
...
procedure TCustomDBGrid2.InternalLayout;
...
begin
  ...
  If FieldCount>0 then i:=1 else i:=0; {jsc}
  inherited FixedCols := i; {jsc}
  FTitleOffset := 0;
  if dgTitles in Options then
  begin
    FTitleOffset := 1;
...
procedure TCustomDBGrid2.LayoutChanged;
begin
  if AcquireLayoutLock then
    EndLayout;
end;

procedure TCustomDBGrid2.LinkActive(Value: Boolean);
var
  Comp: TComponent;
  I: Integer;
begin
  ...
  try
    ...
    begin
      ...
      if (Comp is TCustomDBGrid2)
        and (TCustomDBGrid2(Comp).DragKind = dkDock) then
        Comp.Free;
    end;
    UpdateScrollBar;
...
procedure TCustomDBGrid2.Loaded;
begin
  inherited Loaded;
...
function TCustomDBGrid2.PtInExpandButton(X,Y: Integer; var MasterCol: TColumn): Boolean;
var
  Cell: TGridCoord;
  R: TRect;
begin
...
procedure TCustomDBGrid2.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  Cell: TGridCoord;
...
procedure TCustomDBGrid2.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  Cell: TGridCoord;
...
function TCustomDBGrid2.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
  Result := False;
...
function TCustomDBGrid2.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
  Result := False;
...
procedure TCustomDBGrid2.MoveCol(RawCol, Direction: Integer);
var
  OldCol: Integer;
begin
  FDatalink.UpdateData;
  if RawCol >= ColCount then
    RawCol := ColCount - 1;
  if RawCol < FIndicatorOffset + 1 then RawCol := FIndicatorOffset + 1; {jsc}
  if Direction <> 0 then
  begin
    while (RawCol < ColCount) and (RawCol >= FIndicatorOffset + 1) and {jsc}
      (ColWidths[RawCol] <= 0) do
      Inc(RawCol, Direction);
    if (RawCol >= ColCount) or (RawCol < FIndicatorOffset + 1) then Exit;  {jsc}
  end;
  OldCol := Col;
...
procedure TCustomDBGrid2.Notification(AComponent: TComponent;
...
procedure TCustomDBGrid2.RecordChanged(Field: TField);
var
  I: Integer;
  CField: TField;
begin
...
procedure TCustomDBGrid2.Scroll(Distance: Integer);
var
...
procedure TCustomDBGrid2.SetColumns(Value: TDBGridColumns);
begin
...
procedure TCustomDBGrid2.SetColumnAttributes;
var
  I: Integer;
begin
  for I := 0 to FColumns.Count-1 do
...
procedure TCustomDBGrid2.SetDataSource(Value: TDataSource);
begin
...
procedure TCustomDBGrid2.SetEditText(ACol, ARow: Longint; const Value: string);
begin
  FEditText := Value;
end;

procedure TCustomDBGrid2.SetOptions(Value: TDBGridOptions);
const
...
procedure TCustomDBGrid2.SetSelectedField(Value: TField);
var
  I: Integer;
begin
...
procedure TCustomDBGrid2.SetSelectedIndex(Value: Integer);
begin
  MoveCol(DataToRawColumn(Value), 0);
end;

procedure TCustomDBGrid2.SetTitleFont(Value: TFont);
begin
  FTitleFont.Assign(Value);
  if dgTitles in Options then LayoutChanged;
end;

function TCustomDBGrid2.StoreColumns: Boolean;
begin
  Result := Columns.State = csCustomized;
end;

procedure TCustomDBGrid2.TimedScroll(Direction: TGridScrollDirection);
begin
  if FDatalink.Active then
  begin
    with FDatalink do
...
procedure TCustomDBGrid2.TitleClick(Column: TColumn);
begin
  if Assigned(FOnTitleClick) then FOnTitleClick(Column);
end;

procedure TCustomDBGrid2.TitleFontChanged(Sender: TObject);
begin
...
procedure TCustomDBGrid2.UpdateActive;
var
  NewRow: Integer;
  Field: TField;
begin
...
procedure TCustomDBGrid2.UpdateData;
var
  Field: TField;
begin
  Field := SelectedField;
...
procedure TCustomDBGrid2.UpdateRowCount;
var
  OldRowCount: Integer;
begin
...
procedure TCustomDBGrid2.UpdateScrollBar;
var
...
function TCustomDBGrid2.ValidFieldIndex(FieldIndex: Integer): Boolean;
begin
  Result := DataLink.GetMappedIndex(FieldIndex) >= 0;
end;

procedure TCustomDBGrid2.CMParentFontChanged(var Message: TMessage);
begin
  inherited;
  if ParentFont then
  begin
...
procedure TCustomDBGrid2.CMBiDiModeChanged(var Message: TMessage);
var
  Loop: Integer;
begin
  inherited;
  for Loop := 0 to ComponentCount - 1 do
    if Components[Loop] is TCustomDBGrid2 then
      with Components[Loop] as TCustomDBGrid2 do
        { Changing the window, echos down to the subgrid }
        if Parent <> nil then
          Parent.BiDiMode := Self.BiDiMode;
end;

procedure TCustomDBGrid2.CMExit(var Message: TMessage);
begin
  try
    if FDatalink.Active then
      with FDatalink.Dataset do
...
procedure TCustomDBGrid2.CMFontChanged(var Message: TMessage);
var
  I: Integer;
begin
  inherited;
...
procedure TCustomDBGrid2.CMDeferLayout(var Message);
begin
  if AcquireLayoutLock then
    EndLayout
  else
...
procedure TCustomDBGrid2.CMDesignHitTest(var Msg: TCMDesignHitTest);
var
  MasterCol: TColumn;
begin
  inherited;
...
procedure TCustomDBGrid2.WMSetCursor(var Msg: TWMSetCursor);
begin
...
procedure TCustomDBGrid2.WMSize(var Message: TWMSize);
begin
  inherited;
...
procedure TCustomDBGrid2.WMVScroll(var Message: TWMVScroll);
var
  SI: TScrollInfo;
begin
  if not AcquireFocus then Exit;
...
procedure TCustomDBGrid2.SetIme;
var
  Column: TColumn;
begin
...
procedure TCustomDBGrid2.UpdateIme;
begin
  if not SysLocale.FarEast then Exit;
  SetIme;
  SetImeName(ImeName);
...
procedure TCustomDBGrid2.WMIMEStartComp(var Message: TMessage);
begin
  inherited;
  ShowEditor;
end;

procedure TCustomDBGrid2.WMSetFocus(var Message: TWMSetFocus);
begin
...
procedure TCustomDBGrid2.WMKillFocus(var Message: TMessage);
begin
...
function TCustomDBGrid2.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := (DataLink <> nil) and DataLink.ExecuteAction(Action);
end;

function TCustomDBGrid2.UpdateAction(Action: TBasicAction): Boolean;
begin
  Result := (DataLink <> nil) and DataLink.UpdateAction(Action);
end;

procedure TCustomDBGrid2.ShowPopupEditor(Column: TColumn; X, Y: Integer);
var
  SubGrid: TCustomDBGrid2;
  DS: TDataSource;
  I: Integer;
  FloatRect: TRect;
  Cmp: TControl;
begin
  ...
  for I := 0 to ComponentCount-1 do
    if Components[I] is TCustomDBGrid2 then
    begin
      SubGrid := TCustomDBGrid2(Components[I]);
  ...
  SubGrid := TCustomDBGrid2(TComponentClass(Self.ClassType).Create(Self));
  try
...
//    SubGrid.ManualDock(nil,nil,alClient);
    ...
    Cmp := Self;
    while (Cmp <> nil) and (TCustomDBGrid2(Cmp).PopupMenu = nil) do
      Cmp := Cmp.Parent;
    if Cmp <> nil then
      SubGrid.PopupMenu := TCustomDBGrid2(Cmp).PopupMenu;
...
procedure TCustomDBGrid2.CalcSizingState(X, Y: Integer;
...
function TCustomDBGrid2.CheckColumnDrag(var Origin, Destination: Integer;
...
function TCustomDBGrid2.BeginColumnDrag(var Origin, Destination: Integer;
  const MousePt: TPoint): Boolean;
var
  I, ARow: Integer;
begin
...
function TCustomDBGrid2.EndColumnDrag(var Origin, Destination: Integer;
  const MousePt: TPoint): Boolean;
begin
...
procedure TCustomDBGrid2.InvalidateTitles;
var
  R: TRect;
  DrawInfo: TGridDrawInfo;
begin
...
procedure TCustomDBGrid2.TopLeftChanged;
begin
  InvalidateTitles;
...
procedure Register;     {JSC}
begin
  RegisterComponents('Data Controls', [TDbGrid2]);
end;
