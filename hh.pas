...
{ Special Thanks to Rob Chandler <robertc@helpware.net> for his authorship
  and his permission to distribute this source code along with AQUATOX }
...
var                            //functions are invalid if HHCtrlHandle = 0
  ...
  Procedure HTMLHelpContext(Filen: String); overload;
  Procedure HTMLHelpContext(ContextNumber: Integer); overload;
...
Procedure HTMLHelpContext(Filen: String);
Var PC: PAnsiChar;
Begin
  PC := PChar(HelpFilePath+'::/'+Filen);
  HTMLHelp(0,PC,HH_Display_Topic,0);
End;

Procedure HTMLHelpContext(ContextNumber: Integer);
Var PC: PAnsiChar;
Begin
  PC := PChar(HelpFilePath);
  HTMLHelp(0,PC,HH_Help_Context,ContextNumber);
End;
