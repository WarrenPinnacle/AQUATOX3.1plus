...
Uses
  SysUtils, Windows, Messages, Classes, Graphics, Controls, Global, Uncert, Progress,
  Forms, Dialogs, StdCtrls, Math, OLECtrls, ComObj, SyncObjs;
type
...
function RaiseAssertionFailed:LongInt;
...
begin
  ...
  if not GetFileSizeEx(FHandle, Result)
    then RaiseSystemErrorFmt('GetFileSize failed for %s.', [FFileName]);
  if Result = 0 then RaiseSystemErrorFmt('FileSize Returning Zero for %s.', [FFileName]);
...
begin
  if SetFilePointerEx(FHandle, Offset, @Result, ord(Origin))=0 then begin
