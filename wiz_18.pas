...
var
  WizBase18: TWizBase18;
  PSVar: Array[1..50] of AllVariables;
...
Function HasPSLoad(Vr: AllVariables): Boolean;
Begin
  HasPSLoad := Has_Alt_Loadings(Vr,StV,WaterCol);
End;

Procedure TWizBase18.GetTableData;
    ...
    Begin
      ...
      If CurrentVar=DissRefrDetr
        then Begin
               ...
               LoadingsFromTable(Table1,PDRD.InputRecord.Load.Alt_Loadings[PointSource])
             End
        else LoadingsFromTable(Table1,PSV.LoadsRec.Alt_Loadings[PointSource]);
    End;
...
Procedure TWizBase18.PutTableData;
Var TableIn: TTable;
    i: Integer;
    DynLoad: TLoadings;
    PSV: TStateVariable;
...
Begin
  ...
  If CurrentVar=DissRefrDetr
    then Begin
           PDRD := WizStudy.SV.GetStatePointer(CurrentVar,StV,WaterCol);
           DynLoad := PDRD.InputRecord.Load.Alt_Loadings[PointSource]
         End
    else DynLoad := PSV.LoadsRec.Alt_Loadings[PointSource];
...
Procedure TWizBase18.UpdateScreen;
...
Begin
      ...
      LoadBox.Items.Clear;
       For VarLoop := H2OTox1 to Fish15 do
        Begin
          ...
          If (PSV<>nil) and (HasPSLoad(VarLoop)) and
             (not (varloop in [DissLabDetr..SuspLabDetr])) then
            Begin
              Inc(NumFlows);
              PSVar[NumFlows] := VarLoop;
              ...
              Loadbox.Items.Add('Point-Source '+IDStr);
            End
        End;
      W18_ListWritten := True;
    End;
  ...
  If LoadBox.ItemIndex=-1 then
    Begin
      ...
      PSUnitLabel.Enabled := False;
      PSUnitLabel2.Enabled := False;
      IgnoreAllButt.Visible := False;
      exit;
    End;

  PSUnitLabel.Enabled := True;
  PSUnitLabel2.Enabled := True;
  DynamButt.Enabled := True;
  ConstButt.Enabled := True;
  Panel8.Enabled := True;
  Panel8.Color := clBtnFace;
  CurrentVar := PSVar[LoadBox.ItemIndex+1];
  ...
  ConstCh := ThisLoad.Alt_UseConstant[PointSource];
  ConstButt.Checked := ConstCh;
  DynamButt.Checked := Not ConstCh;
  EnableDisable(nil);

  ConstEdit.Text :=  FloatToStrF(Thisload.Alt_ConstLoad[PointSource],ffgeneral,9,4);
...
Function  TWizBase18.ExecuteScreen: WizOutput;
Begin
  If FirstVisit then w18_DataChanged := False;
  W18_ListWritten := False;
  CurrentVar := NullStateVar;
  DBaseNeedsSaving := False;
  ...
  If (WizStatus=1) and (w18_DataChanged) then WizStatus := 2;

End;


procedure TWizBase18.enabledisable(Sender: TObject);
...
Begin
  ...
  If CurrentVar<>NullStateVar then
    Begin
      ...
      If PThisLoad.Alt_UseConstant[PointSource] <> ConstChosen then w18_DataChanged := True;
      PThisLoad^.Alt_UseConstant[PointSource] := ConstChosen;
    End;
...
procedure TWizBase18.LoadBoxClick(Sender: TObject);
...
procedure TWizBase18.ConstEditExit(Sender: TObject);
Var Conv  : Double;
    Result: Integer;
    Txt: String;
    PSV: TStateVariable;
...
begin
  ...
  If (Result<>0)
    then MessageDlg('Incorrect Numerical Format Entered',mterror,[mbOK],0)
      ...
      Begin
        w18_DataChanged := True;
        PSV := WizStudy.SV.GetStatePointer(CurrentVar,StV,WaterCol);
        If CurrentVar=DissRefrDetr
          then Begin
                 PDRD := WizStudy.SV.GetStatePointer(CurrentVar,StV,WaterCol);
                 PDRD.InputRecord.Load.Alt_ConstLoad[PointSource] := Conv;
               End
          else PSV.LoadsRec.Alt_ConstLoad[PointSource] := Conv;
      End;
  UpdateScreen;
end;

procedure TWizBase18.GridExit(Sender: TObject);
begin
  w18_DataChanged := True;
  DBaseNeedsSaving := True;
end;

procedure TWizBase18.FImportClick(Sender: TObject);
begin
  w18_DataChanged := True;
  DBaseNeedsSaving   := True;
  Table1.Active:=False;
  ImportForm.ChangeLoading(LoadBox.Items[LoadBox.ItemIndex]+' ('+PSUnitLabel2.Caption+')',Table1,False,False,CTNone);
  Table1.Active:=True;
  Update;
end;

procedure TWizBase18.NavClick(Sender: TObject; Button: TNavigateBtn);
begin
  DBaseNeedsSaving := True;
end;

procedure TWizBase18.IgnoreAllButtClick(Sender: TObject);
...
begin
  ...
  If CurrentVar<>NullStateVar then
    Begin
      PSV := WizStudy.SV.GetStatePointer(CurrentVar,StV,WaterCol);
      IF (PSV.LoadsRec.NoUserLoad<>IgnoreChosen) then w18_DataChanged := True;
...
procedure TWizBase18.Button10Click(Sender: TObject);
begin
